...
class Memory:
	...
	def add(self, action_replay, perform_under_sampling=False):
		if perform_under_sampling:
			if action_replay[len(action_replay) - 1] and self.length[0]/self.length[1] <= 1.2:
				self.under_sampled_rewards.append(action_replay)
				self.length[0] += 1
			elif action_replay[len(action_replay) - 1] == 0 and self.length[1]/self.length[0] <= 1.2:
	...
	def sample(self, n, perform_under_sampling=False):
		if perform_under_sampling:
			if self.load:
				print "LOADED", len(self.transition_list)
				self.load = False
	...
	def load_memory(self):
		if os.path.isfile(self.savefile_name):
			...
			print "Memory Loaded", len(self.transition_list)
			self.load = True
