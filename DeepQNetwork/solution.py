...
class Player:
	...
	def train(self, early_evaluate=False):
		itr, avg, max_score = self.load_config()

		try:
			while self.agent.counter < 7000000:
				reward, q_val_list, q_val = self.game.train(self.agent)
	...
	def load_config(self, test=False):
		...
		if os.path.isfile(self.config_filename):
			...
			self.agent.counter = float(config["avg"])
			self.agent.eps = int(config["frame"])
			config_file.close()
			print "Config loaded"
